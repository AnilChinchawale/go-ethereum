...
func (c *XDPoS) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if number%c.config.Epoch == 0 {
		...
		if c.HookPenalty != nil || c.HookPenaltyTIPSigning != nil {
			...
			// TODO: Verify penalties in header match computed penalties
			// This requires the Penalties field in types.Header which isn't in standard geth
		}
...
func (c *XDPoS) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	// TODO: Add double validation check if header.Validator field exists
	// This requires XDC-specific header fields not in standard geth
...
func (c *XDPoS) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	// Handle voting proposals for non-checkpoint blocks
	if number%c.config.Epoch != 0 {
		c.lock.RLock()
	...
	// At checkpoint blocks, add signer list and handle penalties
	...
	if number >= c.config.Epoch && number%c.config.Epoch == 0 {
		if c.HookPenalty != nil || c.HookPenaltyTIPSigning != nil {
			...
			if len(penMasternodes) > 0 {
				...
				// TODO: Set header.Penalties when field exists
			}
		}
...
// Finalize implements consensus.Engine. It applies any post-transaction
// state modifications but does not assemble the block.
func (c *XDPoS) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body) {
	// No block rewards in XDPoS at finalize (handled in FinalizeAndAssemble)
	header.UncleHash = types.CalcUncleHash(nil)
}

// FinalizeAndAssemble implements consensus.Engine, running any post-transaction
...
func (c *XDPoS) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, statedb *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	...
	if rCheckpoint == 0 {
		rCheckpoint = c.config.Epoch // Default to epoch if not set
	}
	...
	if c.HookReward != nil && number%rCheckpoint == 0 {
		...
		// Save rewards to file if configured
...
func (c *XDPoS) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// TODO: Add double validation signature (header.Validator) when field exists
...
// Close implements consensus.Engine. It's a noop for XDPoS.
...
// TODO: This requires header.Validator field which doesn't exist in standard geth
...
// removePenaltiesFromBlock removes penalized masternodes from a specific epoch block
func (c *XDPoS) removePenaltiesFromBlock(chain consensus.ChainHeaderReader, masternodes []common.Address, epochNumber uint64) []common.Address {
	...
	// TODO: Get penalties from block when header.Penalties field exists
...
// compareSignersLists compares two signer lists for equality (ignoring order)
...
// removeItemFromArray removes items from an array
...
// isSigningTransaction checks if a transaction is a signing transaction
// TODO: This needs to be implemented based on XDC transaction types
func isSigningTransaction(tx *types.Transaction) bool {
	// Check if tx is to the block signer contract
	// This is a placeholder - actual implementation needs XDC contract addresses
	return false
}

// wiggleTime is the random delay for out-of-turn signing
